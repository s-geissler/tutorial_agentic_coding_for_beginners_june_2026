...
(function () {
  ...
  // The footer's `.section` slot is filled with the deck title
  // (falls back to the section label if no <title> is set); the
  // current section label is shown in the top nav via #nav-section.
  const deckTitle = (document.title || '').trim();
  slides.forEach((s, i) => {
    const headerSection = s.querySelector('.slide-header .section');
    if (headerSection) headerSection.textContent = deckTitle || sectionLabelFor(s);
